...
router.post('/', requireAuth, async (req: any, res: any) => {
  ...
  const form = await prisma.form.create({ data: { ownerId: req.userId, title } });
  res.json({ form });
});

// Owner: add field
router.post('/:id/fields', requireAuth, async (req: any, res: any) => {
  const { id } = req.params;
  const { key, label, type } = req.body;
  const form = await prisma.form.findUnique({ where: { id } });
  if (!form) return res.status(404).json({ error: 'Not found' });
  if (form.ownerId !== req.userId) return res.status(403).json({ error: 'Forbidden' });
  // determine max order
  const max = await prisma.field.aggregate({ where: { formId: id }, _max: { order: true } })
  const nextOrder = (max._max.order ?? 0) + 1
  const field = await prisma.field.create({ data: { formId: id, key, label, type, order: nextOrder } });
  res.json({ field });
});

// Owner: reorder fields
router.post('/:id/fields/order', requireAuth, async (req: any, res: any) => {
  const { id } = req.params;
  ...
  const tx = order.map((fid:any, idx:number) => prisma.field.update({ where: { id: fid }, data: { order: idx+1 } }))
...
router.get('/:id/submissions', requireAuth, async (req: any, res: any) => {
  const { id } = req.params;
  const form = await prisma.form.findUnique({ where: { id } });
  if (!form) return res.status(404).json({ error: 'Not found' });
  if (form.ownerId !== req.userId) return res.status(403).json({ error: 'Forbidden' });
  const submissions = await prisma.submission.findMany({ where: { formId: id }, orderBy: { createdAt: 'desc' } });
  res.json({ submissions });
});

// Owner: export submissions as CSV (immediate)
router.get('/:id/submissions/export', requireAuth, async (req: any, res: any) => {
  const { id } = req.params;
  const form = await prisma.form.findUnique({ where: { id } });
  if (!form) return res.status(404).json({ error: 'Not found' });
  if (form.ownerId !== req.userId) return res.status(403).json({ error: 'Forbidden' });
  const submissions = await prisma.submission.findMany({ where: { formId: id }, orderBy: { createdAt: 'asc' } });
  // Build CSV
